...
BrowserBot.prototype.doesThisFrameMatchFrameExpression = function(currentFrameString, target) {
  ...
  let currentFrame = Components.utils.evalInSandbox(currentFrameString, mySandbox);
  //var currentFrame;
...
BrowserBot.prototype.modifyWindowToRecordPopUpDialogs = function(originalWindow, browserBot) {
  ...
  windowToModify.prompt = function(message) {
    ...
    let result = !browserBot.nextConfirmResult ? null : browserBot.nextPromptResult;
    browserBot.nextConfirmResult = true;
  ...
  let newOpen = function(url, windowName, windowFeatures, replaceFlag) {
    ...
    let openedWindow = myOriginalOpen(url, windowName, windowFeatures, replaceFlag);
...
BrowserBot.prototype.pollForLoad = function(loadFunction, windowObject, originalDocument, originalLocation, originalHref, marker) {
  ...
  try {
    ...
    let isSamePage = this._isSamePage(windowObject, originalDocument, originalLocation, originalHref, marker);
...
BrowserBot.prototype.recursivelyDeleteCookie = function(cookieName, domain, path, win) {
  ...
  let deleted = this._recursivelyDeleteCookie(cookieName, "." + domain, path, doc);
  if (deleted) return;
...
BrowserBot.prototype.replaceText = function(element, stringValue) {
  ...
  if (getTagName(element) == "body") {
    if (element.ownerDocument && element.ownerDocument.designMode) {
      let designMode = new String(element.ownerDocument.designMode).toLowerCase();
      if (designMode == "on") {
...
BrowserBot.prototype._getFrameFromGlobal = function(target) {
  ...
  let frameElement = this.findElementBy("implicit", target, this.topFrame.document, this.topFrame);
  if (frameElement) {
