...
describe("<Console />", () => {
  ...
  it("should not display an unread notification on the log tab when it is in focus and the log's been updated", () => {
    const { container } = renderIntoDocument(<Console />);
    Logger.log("blah");
    expect(container).not.toBe(null);
  });
  it.skip("should display an unread notification on the log tab when the reference tab is in focus and the log has been updated)", () => {
    const { container } = renderIntoDocument(<Console />);
