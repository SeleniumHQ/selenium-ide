...
async function emitTest(
  ...
  result += render(await hooks.inEachBegin.emit({ tests, isOptional: true }), {
  ...
  result += render(await hooks.inEachEnd.emit({ tests, isOptional: true }), {
    startingLevel: commandLevel,
  })
...
async function emitSuite(
  ...
  result += render(await hooks.declareDependencies.emit({ tests }))
  result += render(suiteDeclaration, { startingLevel: suiteLevel })
  result += render(await hooks.declareVariables.emit({ tests }), {
    startingLevel: testLevel,
  })
  result += render(await hooks.beforeAll.emit({ tests, isOptional: true }), {
    startingLevel: testLevel,
  })
  result += render(await hooks.beforeEach.emit({ tests }), {
    startingLevel: testLevel,
  })
  result += render(await hooks.afterEach.emit({ tests }), {
    startingLevel: testLevel,
  })
  result += render(await hooks.afterAll.emit({ tests, isOptional: true }), {
    startingLevel: testLevel,
  })
  result += render(
    await hooks.declareMethods.emit({ tests, isOptional: true }),
    {
      startingLevel: testLevel,
    }
  )
