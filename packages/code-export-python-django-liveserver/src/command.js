...
function emitWaitForWindow() {
  ...
  const commands = [
    ...
    { level: 0, statement: 'wh_now = self.selenium.window_handles' },
    {
      level: 0,
...
function emitAssertAlert(alertText) {
  return Promise.resolve(
    `assert self.selenium.switch_to.alert.text == "${alertText}"`
  )
}

function emitAnswerOnNextPrompt(textToSend) {
  const commands = [
    { level: 0, statement: 'alert = self.selenium.switch_to.alert' },
...
async function emitCheck(locator) {
  const commands = [
    {
      level: 0,
      statement: `element = self.selenium.find_element(${await location.emit(
        locator
      )})`,
    },
    {
...
function emitChooseCancelOnNextConfirmation() {
  return Promise.resolve(`self.selenium.switch_to.alert.dismiss()`)
}

function emitChooseOkOnNextConfirmation() {
  return Promise.resolve(`self.selenium.switch_to.alert.accept()`)
}

async function emitClick(target) {
  return Promise.resolve(
    `self.selenium.find_element(${await location.emit(target)}).click()`
  )
}

async function emitClose() {
  return Promise.resolve(`self.selenium.close()`)
}

function generateExpressionScript(script) {
  const scriptString = script.script.replace(/"/g, "'")
  return `self.selenium.execute_script("return (${scriptString})"${generateScriptArguments(
    script
  )})`
}
...
async function emitDragAndDrop(dragged, dropped) {
  const commands = [
    {
      level: 0,
      statement: `dragged = self.selenium.find_element(${await location.emit(
        dragged
      )})`,
    },
    {
      level: 0,
      statement: `dropped = self.selenium.find_element(${await location.emit(
        dropped
      )})`,
    },
    { level: 0, statement: 'actions = ActionChains(driver)' },
...
async function emitEditContent(locator, content) {
  const commands = [
    ...
    {
      level: 0,
      statement: `self.selenium.execute_script("if(arguments[0].contentEditable === 'true') {arguments[0].innerText = '${content}'}", element)`,
    },
  ]
  return Promise.resolve({ commands })
}
...
async function emitExecuteScript(script, varName) {
  const scriptString = script.script.replace(/"/g, "'")
  const result = `self.selenium.execute_script("${scriptString}"${generateScriptArguments(
    script
  )})`
...
async function emitExecuteAsyncScript(script, varName) {
  const result = `self.selenium.execute_async_script("var callback = arguments[arguments.length - 1];${
    script.script
...
async function emitMouseOut() {
  const commands = [
    {
      level: 0,
      statement: `element = self.selenium.find_element(By.CSS_SELECTOR, "body")`,
    },
    { level: 0, statement: 'actions = ActionChains(driver)' },
...
function emitOpen(target) {
  const url = /^(file|http|https):\/\//.test(target)
    ? `"${target}"`
    : `self.live_server_url + "${target}"`
  return Promise.resolve(`self.selenium.get(${url})`)
}
...
async function emitRunScript(script) {
  return Promise.resolve(
    `self.selenium.execute_script("${script.script}${generateScriptArguments(
      script
    )}")`
  )
}
...
async function emitSetWindowSize(size) {
  ...
  return Promise.resolve(`self.selenium.set_window_size(${width}, ${height})`)
}
...
async function emitSelect(selectElement, option) {
  const commands = [
    {
      level: 0,
      statement: `dropdown = self.selenium.find_element(${await location.emit(
        selectElement
      )})`,
    },
    {
...
async function emitSelectFrame(frameLocation) {
  if (frameLocation === 'relative=top' || frameLocation === 'relative=parent') {
    return Promise.resolve('self.selenium.switch_to.default_content()')
  } else if (/^index=/.test(frameLocation)) {
    return Promise.resolve(
      `self.selenium.switch_to.frame(${Math.floor(
    ...
    return Promise.resolve({
      commands: [
        ...
        { level: 0, statement: 'self.selenium.switch_to.frame(element)' },
      ],
    })
  }
}
...
async function emitSelectWindow(windowLocation) {
  if (/^handle=/.test(windowLocation)) {
    return Promise.resolve(
      `self.selenium.switch_to.window(${windowLocation.split('handle=')[1]})`
    )
  } else if (/^name=/.test(windowLocation)) {
    return Promise.resolve(
      `self.selenium.switch_to.window("${windowLocation.split('name=')[1]}")`
    )
    ...
    if (windowLocation === 'win_ser_local') {
      ...
      return Promise.resolve({
        commands: [
          {
            level: 0,
            statement: `self.selenium.switch_to.window(self.selenium.window_handles[${index}])`,
          },
        ],
      })
    }
...
async function emitSendKeys(target, value) {
  return Promise.resolve(
    `self.selenium.find_element(${await location.emit(
      target
...
async function emitStoreAttribute(locator, varName) {
  ...
  const commands = [
    {
      level: 0,
      statement: `attribute = self.selenium.find_element(${await location.emit(
        elementLocator
...
async function emitStoreText(locator, varName) {
  const result = `self.selenium.find_element(${await location.emit(
    locator
  )}).text`
  return Promise.resolve(variableSetter(varName, result))
}
...
async function emitStoreTitle(_, varName) {
  return Promise.resolve(variableSetter(varName, 'self.selenium.title'))
}
...
async function emitStoreWindowHandle(varName) {
  return Promise.resolve(
    variableSetter(varName, 'self.selenium.current_window_handle')
  )
}

async function emitStoreXpathCount(locator, varName) {
  const result = `len(self.selenium.find_elements(${await location.emit(
    locator
  )}))`
  return Promise.resolve(variableSetter(varName, result))
}
...
async function emitSubmit(_locator) {
  return Promise.resolve(
    `raise Exception("\`submit\` is not a supported command in Selenium Webself.selenium. Please re-record the step in the IDE.")`
  )
}
...
async function emitVerifyChecked(locator) {
  return Promise.resolve(
    `assert self.selenium.find_element(${await location.emit(
      locator
    )}).is_selected() is True`
  )
}
...
async function emitVerifyElementPresent(locator) {
  const commands = [
    {
      level: 0,
      statement: `elements = self.selenium.find_elements(${await location.emit(
        locator
      )})`,
    },
...
async function emitVerifyNotSelectedValue(locator, expectedValue) {
  const commands = [
    {
      level: 0,
      statement: `value = self.selenium.find_element(${await location.emit(
        locator
      )}).get_attribute("value")`,
    },
    {
...
async function emitVerifyNotText(locator, text) {
  const commands = [
    {
      level: 0,
      statement: `text = self.selenium.find_element(${await location.emit(
        locator
      )}).text`,
    },
...
async function emitVerifyText(locator, text) {
  const commands = [
    {
      level: 0,
      statement: `assert self.selenium.find_element(${await location.emit(
        locator
...
async function emitVerifyTitle(title) {
  return Promise.resolve(`assert self.selenium.title == "${title}"`)
}
...
async function emitWaitForElementEditable(locator, timeout) {
  const commands = [
    {
      level: 0,
      statement: `WebDriverWait(self.selenium, ${timeout}).until(expected_conditions.element_to_be_clickable((${await location.emit(
        locator
      )})))`,
    },
  ]
...
async function emitWaitForElementPresent(locator, timeout) {
  const commands = [
    {
      level: 0,
      statement: `WebDriverWait(self.selenium, ${timeout}).until(expected_conditions.presence_of_element_located((${await location.emit(
        locator
      )})))`,
    },
  ]
...
async function emitWaitForElementVisible(locator, timeout) {
  const commands = [
    {
      level: 0,
      statement: `WebDriverWait(self.selenium, ${timeout}).until(expected_conditions.visibility_of_element_located((${await location.emit(
        locator
      )})))`,
    },
  ]
...
async function emitWaitForElementNotEditable(locator, timeout) {
  const commands = [
    {
      level: 0,
      statement: `WebDriverWait(self.selenium, ${timeout}).until_not(expected_conditions.element_to_be_clickable((${await location.emit(
        locator
      )})))`,
    },
  ]
...
async function emitWaitForElementNotPresent(locator, timeout) {
  const commands = [
    {
      level: 0,
      statement: `WebDriverWait(self.selenium, ${timeout}).until(expected_conditions.invisibility_of_element_located((${await location.emit(
        locator
      )})))`,
    },
  ]
