...
export const displayName = 'Node New Relic Synthetics'


global.nrsynthetics= synthetics.commands()
...
function generateMethodDeclaration(name) {
  ...
  const command =
    `function ${exporter.parsers.sanitizeName(name)}(){\n` +
    `\treturn Promise.resolve(true)`
  return command
}
...
export async function emitTest({
  ...
  let tmpOpts = Object.assign({}, opts, {terminatingKeyword: `${tcFooter}})\n`})
  var result = await exporter.emit.test(test, tests, {
    ...tmpOpts,
    testDeclaration,
    enableOriginTracing,
    project,
  ...
  tmpOpts = Object.assign({}, opts, {terminatingKeyword: ''})
...
export async function emitSuite({
  ...
  var result = await exporter.emit.testsFromSuite(tests, suite, tmpOpts, {
    enableOriginTracing,
  ...
  for ( let key in result){
    let {commands, testEnd} = result[key]
