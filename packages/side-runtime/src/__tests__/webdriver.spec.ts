...
describe('webdriver executor', () => {
  it('should implement all the Selenium commands', () => {
    Object.keys(Commands).forEach((command) => {
      if (!(command in ControlFlowCommandNames)) {
        expect(() => {
          if (
  ...
  describe.each([
    ...
    describe('plugins', () => {
      it('should be able to register a command', async () => {
        ...
        executor.registerCommand(
          'commandName',
          async function (this: WebDriverExecutor, target, value) {
    ...
    describe('add selection', () => {
      ...
      it('should add to a multi select element', async () => {
        ...
        expect(
          (
            await driver.executeScript<any[]>(
        ...
        const selections = await driver.executeScript<any[]>(
          'return arguments[0].selectedOptions',
          element
        )
