...
async function emitDoubleClick(target) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      target
    )}).then(element => {driver.actions().doubleClick(element).perform();});`
  )
}
...
async function emitDragAndDrop(dragged, dropzone) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      dragged
    )}).then(dragged => {driver.findElement(${await LocationEmitter.emit(
      dropzone
    )}).then(dropzone => {driver.actions().dragAndDrop(dragged, dropzone).perform();});});`
  )
}
...
async function emitType(target, value) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      target
    )}).then(element => {element.clear().then(() => {element.sendKeys(\`${value}\`);});});`
  )
}
...
async function emitCheck(locator) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(element => { element.isSelected().then(selected => {if(!selected) { element.click();}}); });`
  )
}
...
async function emitVerifySelectedLabel(locator, labelValue) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(element => {element.getAttribute("value").then(selectedValue => {element.findElement(By.xpath('option[@value="'+selectedValue+'"]')).then(selectedOption => {selectedOption.getText().then(selectedLabel => {expect(selectedLabel).toBe(\`${labelValue}\`);});});});});`
  )
}
...
async function emitVerifyNotText(locator, text) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(element => {element.getText().then(text => {expect(text).not.toBe(\`${text}\`)});});`
  )
}

async function emitVerifyTitle(title) {
  return Promise.resolve(
    `await driver.getTitle().then(title => {expect(title).toBe(\`${title}\`);});`
  )
}
...
async function emitStoreText(locator, varName) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(element => {element.getText().then(text => {vars["${varName}"] = text;});});`
  )
}
...
async function emitStoreValue(locator, varName) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(element => {element.getAttribute("value").then(value => {vars["${varName}"] = value;});});`
  )
}

async function emitStoreTitle(_, varName) {
  return Promise.resolve(
    `await driver.getTitle().then(title => {vars["${varName}"] = title;});`
  )
}
...
async function emitStoreXpathCount(locator, varName) {
  return Promise.resolve(
    `await driver.wait(until.elementsLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(elements => {vars["${varName}"] = elements.length;});`
  )
}
...
async function emitStoreAttribute(locator, varName) {
  ...
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      elementLocator
    )}).then(element => element.getAttribute("${attributeName}").then(attribute => {vars["${varName}"] = attribute;}));`
  )
}
...
async function emitSelect(selectElement, option) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      selectElement
    )}).then(element => {element.findElement(${await SelectionEmitter.emit(
      option
...
async function emitMouseMove(locator) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(element => {driver.actions().mouseMove(element).perform();});`
  )
}
...
async function emitMouseOut(locator) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(element => {driver.actions().mouseMove(element, {x: -1, y: -1}).perform();});`
  )
}

function emitAssertAlert(alertText) {
  return Promise.resolve(
    `await driver.switchTo().alert().then(alert => {alert.getText().then(text => {expect(text).toBe(\`${alertText}\`);});});`
  )
}

function emitAssertAlertAndAccept(alertText) {
  return Promise.resolve(
    `await driver.switchTo().alert().then(alert => {alert.getText().then(text => {expect(text).toBe(\`${alertText}\`);alert.accept();});});`
  )
}
...
function emitAnswerOnNextPrompt(textToSend) {
  return Promise.resolve(
    `await driver.switchTo().alert().then(alert => {alert.sendKeys(\`${textToSend}\`).then(() => {alert.accept();});});`
  )
}
...
async function emitEditContent(locator, content) {
  return Promise.resolve(
    `await driver.wait(until.elementLocated(${await LocationEmitter.emit(
      ...
      locator
    )}).then(element => {driver.executeScript(\`if(arguments[0].contentEditable === 'true') {arguments[0].innerHTML = '${content}'}\`, element);});`
  )
}
