...
describe("Selenium code serializer", () => {
  ...
  it("should add a before and after code", async () => {
    ...
    RegisterSuiteHook(hook);
    return expect((await Selianize(project)).suites[0].code).toMatch(/describe\("aaa suite", \(\) => {beforeAll\(async \(\) => {before all code}\);beforeEach\(async \(\) => {before code}\);afterEach\(async \(\) => {after code}\);afterAll\(async \(\) => {after all code}\);it\(/);
  });
  ...
  it("hook should add setup and teardown code", async () => {
    ...
    RegisterTestHook(hook);
    return expect((await Selianize(project)).suites[0].code).toMatch(/it\("aa playback", async \(\) => {some setup codeawait.*other teardown code.*}\);/);
  });
  it("should register a new command emitter", async () => {
    ...
    return expect((await Selianize(project)).tests[0].code).toMatch(/some new command code/);
  });
